...
localparam AXI_CONFIG_DATA_WIDTH = NB_FFT2_CONFIG; //! width of the config data for the fft core
localparam AXI_CONFIG_NFFT_WIDTH = 5;  //! width of the fft lenght field
localparam AXI_CONFIG_FWIN_WIDTH = 1;  //! width of the forward/inverse fft field
localparam AXI_CONFIG_SCAL_WIDTH = 8; //! width of the scaling schedule field

localparam RAM_ROWS = NP; //! RAM Rows Parameter
localparam RAM_COLS = P_MAX; //! RAM Columns Parameter max P used
...
logic                              r_ram_read_col_addr_out2; // ADDEEEEEEEEEEEEEEEEEEEEEEED
...
// AL CDEMOD_REG LE SAQUE EL -1 PORQ NECESITO CONTAR HASTA NP = 32
logic [RAM_ROW_ADDR_WIDTH  : 0] r_ram_read_row_addr_cdemod_reg, r_ram_read_row_addr_cdemod_next; //! RAM Reference Read Address (CDEMOD) 
...
logic [10:0] r_demprod_index_lim;
...
// assign r_ram_read_row_addr_cdemod_out  = (r_ram_read_row_addr_cdemod_reg == NP - 1) ? '1 : '0;
/// CAMBIO PORQ NO SE HACIA EL ULTIMO PRODUCTO !!!
assign r_ram_read_row_addr_cdemod_out  = (r_ram_read_row_addr_cdemod_reg == NP) ? '1 : '0;
...
assign r_alpha_index_check_flag = (r_alpha_index_reg[RAM_ROW_ADDR_WIDTH] == 1'b1) ? '1 : '0;
...
// added for p point fft size
always_comb begin : p_point_fft_size
    case (i_p_frames)
...
always_comb begin : scale_sch_fft_size
    case (i_p_frames)
        8'b10000000 : SCALE_SCH = 8'b01011011;
        8'b01000000 : SCALE_SCH = 8'b00011011;
        8'b00100000 : SCALE_SCH = 8'b00001011;
        8'b00010000 : SCALE_SCH = 8'b00000111;
        8'b00001000 : SCALE_SCH = 8'b00000011;
        default: SCALE_SCH = 8'b01011011;
    endcase
end

// fft runtime config
// assign SCALE_SCH = 10'b1010101011; // select appropiate scaling factor 
// assign SCALE_SCH = 8'b10101011; // select appropiate scaling factor
assign FWD_INV = 1'b1; // select forward fft mode 
// assign SCALE_SCH = 8'b01010111; 
...
// entro con 16,15 y 8,7 entonces el output completo es 25,22
...
localparam NBI_CMULT1_TRUNC = (NB_CMULT1_FULLR - 20) - ((NB_CMULT1_TRUNC - 15) - 1);

logic signed [24 : 0] i_m_cmult1_axis_dout_tdata_re, i_m_cmult1_axis_dout_tdata_im;
logic signed [15 : 0] truncated_cmult1_dout_re, truncated_cmult1_dout_im;
...
// buffer to load before phase correction 
always_ff @(posedge clock) begin : pre_downshift_buffer_load
    if (w_pre_downshift_buffer_load_en) begin
...
always_ff @(posedge clock) begin : load_cdemod_buffer
    if (w_xdem_buffer_load_en) r_xdem_prod[r_ram_read_col_addr_reg - 1] <= i_ram_col_data_out[r_ram_read_row_addr_cdemod_reg]; // i_ram_col_data_out[r_ram_read_row_addr_reg];   
...
always_ff @(posedge clock) begin : load_cdemod_conj_buffer
    if (w_xdem_conj_buffer_load_en) begin
...
// agregado para no perder resolucion en los bits de salida, a ver si con esto detecta mejor o no
...
//assign truncated_cmult2_dout_re = (~|i_m_cmult2_axis_dout_tdata_re[NB_CMULT2_FULLR - 1 -: NBI_CMULT2_TRUNC] || &i_m_cmult2_axis_dout_tdata_re[NB_CMULT2_FULLR - 1 -: NBI_CMULT2_TRUNC]) ?  {i_m_cmult2_axis_dout_tdata_re[NB_CMULT2_FULLR - NBI_CMULT2_TRUNC -: NB_CMULT2_TRUNC],1'b0} : (i_m_cmult2_axis_dout_tdata_re[NB_CMULT2_FULLR - 1]) ? {1'b1,{NB_CMULT2_TRUNC - 1{1'b0}}} : {1'b0,{NB_CMULT2_TRUNC - 1{1'b1}}};
//assign truncated_cmult2_dout_im = (~|i_m_cmult2_axis_dout_tdata_im[NB_CMULT2_FULLR - 1 -: NBI_CMULT2_TRUNC] || &i_m_cmult2_axis_dout_tdata_im[NB_CMULT2_FULLR - 1 -: NBI_CMULT2_TRUNC]) ?  {i_m_cmult2_axis_dout_tdata_im[NB_CMULT2_FULLR - NBI_CMULT2_TRUNC -: NB_CMULT2_TRUNC],1'b0} : (i_m_cmult2_axis_dout_tdata_im[NB_CMULT2_FULLR - 1]) ? {1'b1,{NB_CMULT2_TRUNC - 1{1'b0}}} : {1'b0,{NB_CMULT2_TRUNC - 1{1'b1}}};

// new ver
// get output samples from complex multiplier 2
always_ff @(posedge clock) begin : get_cmult2_samples
    if (i_m_cmult2_axis_dout_tvalid && state_reg == S_CDEMOD_PROD) begin
        ...
        r_fft2_input_buffer[0] <= {truncated_cmult2_dout_im,truncated_cmult2_dout_re};//i_m_cmult2_axis_dout_tdata;
    end
    else begin
    ...
    case (state_reg)
        ...
        w_cmult2_input_en        = '0;
            ...
            if (r_ram_write_addr_out) begin // all blocks stored
                r_ram_write_addr_inc = '0; 
                r_ram_write_addr_clr = '1;
                //w_exp_rom_en         = '1; // added exp_rom_en one clock earlier for better sync
        ...
        S_DOWN_SHIFT : begin
            ...
            // added
            w_pre_downshift_buffer_load_en = '0;
            w_cmult1_input_en              = '1; //  !!1
            // -----
        
            // cmult1 input management
            r_ram_read_col_addr_inc = '0;
            if (w_cmult1_input_en) begin
            // else no es necesario porq el cmult1_input_en viene del otro estado 
                ...
                if (r_ds_index_out) begin 
                    //w_exp_rom_en             = '0; // disable exp rom, goes back to en in s_load_buff
                    r_ds_index_inc           = '0;
                    r_ds_index_clr           = '1;
                    // added
                    w_cmult1_a_sample_ld     = '0; // load data a en
                    w_cmult1_b_sample_ld     = '0; // load data b en
                    // -----
                    w_s_cmult1_axis_a_tvalid = '0;
                    w_s_cmult1_axis_b_tvalid = '0;
            ...
            if (i_m_cmult1_axis_dout_tvalid) begin
                //w_get_cmult1_samples    = '1;
                r_cmult1_dout_count_inc = '1;
            end
            else begin
                //w_get_cmult1_samples    = '0;
                r_cmult1_dout_count_inc = '0;
            end
        ...
        // ADDED TO SOLVE SYNC PROBLEM
        // RESET imod4 counter
        w_reset_imod4_count = '1;
        //
        w_cmult2_input_en        = '0;
        w_cmult1_input_en        = '0;
        r_ds_index_clr           = '1; // addeddddddddddd
        ...
        w_cmult1_input_en        = '0;
            ...
            // cuando entro aca, cargo en el buffer el dato de la ram addr q traia de antes
            w_xdem_buffer_load_en          = '1; // load buffer en
            w_cemod_index_en               = '1; // !!!
            
            // r_ram_read_row_addr_cdemod_inc = '1; ULTIMO UPDATE LO MOVI A WAIT NEXT BLOCKKKKKKKKKKKKKKKKKK
            ...
            if (r_ram_read_row_addr_cdemod_out) begin
                ...
                // la reseteo para cargar desde el 0 los conj en el prox estado
                // reset read address to load conjugate complex demodulate
                // r_ram_read_row_addr_clr        = '1; // !!!!!!!!!!!!!!!!!!!!!!!!
                r_ram_read_row_addr_cdemod_clr = '0;
        ...
        w_cmult1_input_en        = '0;
            ...
            // ADDED !!!!!!!!!!!
            // aca tengo q leer todas las cols y quedarme con el valor de row que quiero
            r_ram_read_col_addr_inc = '1;
        ...
        w_cmult1_input_en        = '0;
            ...
            // r_ram_read_row_addr_inc = '1; // ULTIMO UPDATE, MOVI ESTO AL PROX ESTADO !!!!!!!!!
            
            w_alpha_index_en        = '1; // falta agregar el default
            // por ej aca lo hago asi o puedo poner q el next sea if (state = x) tanto...
            // esta pregunta es para como calcular el alpha_index_next !!
        ...
        S_CDEMOD_PROD : begin
            ...
            if (i_m_cmult2_axis_dout_tvalid) begin
                //w_get_cmult2_samples    = '1;
                r_cmult2_dout_count_inc = '1;
            end
            else begin
                //w_get_cmult2_samples    = '0;
                r_cmult2_dout_count_inc = '0;
            end
            ...
            if (r_cmult2_dout_count_out) begin
                ...
                r_cmult2_dout_count_clr = '0;
                //r_cmult2_dout_count_inc = '1; // dudo
                state_next              = S_CDEMOD_PROD;
            end
        end
        ...
        r_ram_read_row_addr_inc = '1; // ULTIMO UPDATE, MOVI ESTO ACA PORQ ANTES NO ESTABA CALCULANDO BIEN
        
            w_ram_portb_col_en        = '1;
            //r_ram_read_col_addr_inc = '1; /// !!!! added
            ...
            if (r_ram_read_row_addr_out) begin // once np conj
                ...
                r_ram_read_col_addr_inc = '1; ///  ADDEDEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
                
                r_ram_read_row_addr_cdemod_inc = '1; // ULTIMO UPDATE LO MOVI ACAAAAAAAAAAAA
                
                state_next             = S_LOAD_CDEMOD; // se carga el prox no conj
            end
...
// exp rom
//assign o_exp_rom_addr = r_exp_rom_addr_reg;
//assign o_exp_rom_en   = r_exp_rom_en;
