...
localparam NB_CMULT1_A = 2*NB_DATA;
localparam NB_CMULT1_B = NB_EXPO;
localparam NB_CMULT1_O = 2*NB_DATA;
localparam NB_CMULT2_A = 2*NB_DATA;
localparam NB_CMULT2_B = 2*NB_DATA;
localparam NB_CMULT2_O = 4*NB_DATA; // CHANGED TO 4 WAS 2 BEFORE !!
...
//localparam EXP_ROM_SIZE   = NP*P;                 
//localparam EXP_ADDR_WIDTH = $clog2(EXP_ROM_SIZE); 
//localparam EXP_DOUT_WIDTH = NB_EXPO;
...
//logic [RAM_DATA_WIDTH     - 1 : 0] w_ram_row_data_out [RAM_COLS];
...
// exp rom
//logic [EXP_ADDR_WIDTH - 1 : 0] w_exp_rom_addr;
//logic                          w_exp_rom_enable;
//logic [EXP_DOUT_WIDTH - 1 : 0] w_exp_rom_data_out;

// cmult1
// testttt
logic [64 - 1 : 0] w_m_cmult1_axis_dout_tdata; 
...
// cmult2
logic [80 - 1 : 0] w_m_cmult2_axis_dout_tdata; 
...
//exponential_rom # (
//    .N_SAMPLES (NP      ), // number of samples per block
//    .P_BLOCKS  (P       ), // number of blocks
//    .NB_DATA   (NB_EXPO )) // real + imag data width
//downshift_exponential_rom (
//    .clock      (clock             ),  // input sys clock
//    .i_enable   (w_exp_rom_enable  ),  // input wire enable
//    .i_addr     (w_exp_rom_addr    ),  // input wire read address
//   .o_exp_data (w_exp_rom_data_out)); // output wire data out
...
//ram2d_col_write_multi_read # (
//    .ROWS (RAM_ROWS      ), // number of RAM rows
//    .COLS (RAM_COLS      ), // number of RAM cols
//    .W    (RAM_DATA_WIDTH)) // data width
//ram_module (
//    .clk         (clock                 ), // input wire clock      
  //  .wr_port_en  (w_ram_porta_enable    ), // input wire port_enable (port a)         
  //  .wr_col_en   (w_ram_write_enable    ), // input wire write enable (port a)
  //  .wr_col      (w_ram_write_addr      ), // input wire [log2(COLS)-1:0] write_address
  //  .wr_col_data (w_ram_data_in_reg     ), // input wire [W-1:0] data_in [ROWS]
  //  .rd_row_en   (w_ram_portb_row_enable), // input wire read_row_enable (port b, FFT) 
  //  .rd_row      (w_ram_read_row_addr   ), // input wire [log2(ROWS)-1:0] read_row_address
  //  .rd_col_en   (w_ram_portb_col_enable), // input wire read_col_enable (por b, downshift)
  //  .rd_col      (w_ram_read_col_addr   ), // input wire [log2(COLS)-1:0] read_col_address
  //  .rd_row_data (w_ram_row_data_out    ), // output wire [W-1:0] row_data_out [COLS]
  //  .rd_col_data (w_ram_col_data_out    )); // output wire [W-1:0] col_data_out [ROWS]
...
second_stage_fsmd_inst (
    ...
    //.i_ram_row_data_out          (w_ram_row_data_out         ),
    ...
    //.o_exp_rom_addr              (w_exp_rom_addr             ),
    //.o_exp_rom_en                (w_exp_rom_enable           ),
    //.i_exp_rom_dout              (w_exp_rom_data_out         ),
