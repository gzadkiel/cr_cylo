module pm_top_mux #(
	...
	input  logic i_noise_data_valid,  // del PS para avisar OK
	input  logic i_signal_data_valid, // del PS para avisar OK
    ...
    output logic                     o_est_done,
    output logic                     o_detection_done,
    output logic                     o_threshold_est_done // PARA AVISARLE AL PS
);
  
localparam NB_DATA_WIN_O  = 16; 
localparam NBF_DATA_WIN_O = 13;

localparam L     = NP_SAMPLES / 4; // decimation parameter
localparam L_DIV = $clog2(L);

localparam SCD_DATA_OUT = 64;

// last update
logic [$clog2(P_MAX) : 0] r_p_frames;
assign r_p_frames = i_data_size >> L_DIV; // P_FRAMES = N_DESIRED / L; number of frames 

logic [$clog2(N_DESIRED) : 0] r_n_tot;
assign r_n_tot = ((r_p_frames - 1'b1) << L_DIV) + 11'b00001000000; // N_TOT = (P_FRAMES - 1)*L + NP_SAMPLES; number of total samples required
//-----------
...
logic                        w_threshold_est_done; // added

logic                     w_detection_done; // added
logic [NB_DETECT - 1 : 0] w_detect_counter;
logic                     w_compare_detect_en;
logic                     w_det_counter_reset;

logic w_scd_reset; // added to reset estimator after runs
...
pm_top_mux_fsmd_control (
    ...
    .i_scd_estimate_done     (w_detection_done       ), // antes aca estaba el estimation_done
