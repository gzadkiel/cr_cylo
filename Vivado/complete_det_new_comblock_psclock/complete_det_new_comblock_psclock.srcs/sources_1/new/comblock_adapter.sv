module comblock_adapter #(
    ...
    output logic [8  : 0] top_mux_noise_size,
...
logic [8                  : 0] r_top_mux_noise_size, w_top_mux_noise_size;
...
assign w_top_mux_noise_size = i_cb_OUT_REGS_0_reg0_o[23 : 15];
assign w_top_mux_window_sel = i_cb_OUT_REGS_0_reg0_o[25 : 24];
assign w_top_mux_pfa_value  = i_cb_OUT_REGS_0_reg0_o[29 : 26];

// ?? Results into IN_REG0 ??
assign o_cb_IN_REGS_0_reg0_i = {{(32 - 15 - 1 - 1){1'b0}}, 
                               top_mux_threshold_est_done, 
                               top_mux_detection_done    ,
                               top_mux_detect            };
    
// FIFO ? i_data_re / i_data_im
// control of fifo read in signal, CHECK !!!!!!!!!!!!!!!!!
logic w_fifo_read_en;

always_comb begin : check_fifo_empty_read_enable
    if (i_cb_OUT_FIFO_0_fifo_empty_o) w_fifo_read_en = '0;
    else w_fifo_read_en = '1;
end

// fifo output read enable
assign o_cb_OUT_FIFO_0_fifo_re_i = '1 & w_fifo_read_en; 
// ------------------------------------------------------
...
always_ff @(posedge clock) begin : get_fifo_samples
    ...
    else if ((i_cb_OUT_FIFO_0_fifo_valid_o) && (!i_cb_OUT_FIFO_0_fifo_empty_o) && (top_mux_signal_data_valid || top_mux_noise_data_valid)) begin
        i_cb_OUT_FIFO_0_fifo_data_im <= i_cb_OUT_FIFO_0_fifo_data_o[NB_SAMPLE_DATA - 1 : NB_SAMPLE_DATA / 2];
        i_cb_OUT_FIFO_0_fifo_data_re <= i_cb_OUT_FIFO_0_fifo_data_o[NB_SAMPLE_DATA/2 - 1 : 0];
    end
    else begin
        i_cb_OUT_FIFO_0_fifo_data_im <= '0;
        i_cb_OUT_FIFO_0_fifo_data_re <= '0;
