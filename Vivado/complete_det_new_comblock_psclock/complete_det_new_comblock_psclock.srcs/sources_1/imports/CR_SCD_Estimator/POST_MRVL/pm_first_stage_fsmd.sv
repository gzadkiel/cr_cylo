module first_stage_fsmd #(
    ...
    // input  logic signed [NB_DATA          - 1 : 0] i_ram_data_out_re [NP],
    // input  logic signed [NB_DATA          - 1 : 0] i_ram_data_out_im [NP],
...
localparam ADDR_WIDTH = $clog2(P_MAX);  //! ram address width usign Pmax

localparam AXI_CONFIG_DATA_WIDTH = 7; //! width of the config data for the fft core
...
localparam AXI_CONFIG_SCAL_WIDTH = 6; //! width of the scaling schedule field
...
logic [AXI_CONFIG_DATA_WIDTH - 1 : 0] w_s_axis_config_tdata; //! initial fft core config: nfft, fwd/inv and scale_sch

// fft input buffer sigs
logic                             r_fft_input_buffer_ld;
logic        [2*NB_DATA  - 1 : 0] r_fft_input_buffer [NP];
// logic signed [NB_DATA    - 1 : 0] r_fft_input_buffer_re [NP]; //! input buffer to store real data block for fft input
// logic signed [NB_DATA    - 1 : 0] r_fft_input_buffer_im [NP]; //! input buffer to store imag data block for fft input
...
always_ff @(posedge clock) begin
    if (r_fft_input_buffer_ld) begin
        for (int ptr_buffer = 0 ; ptr_buffer < NP ; ptr_buffer = ptr_buffer + 1) begin
            r_fft_input_buffer[ptr_buffer] = i_ram_data_out[ptr_buffer];
            // r_fft_input_buffer_re[ptr_buffer] = i_ram_data_out_re[ptr_buffer];
            // r_fft_input_buffer_im[ptr_buffer] = i_ram_data_out_im[ptr_buffer];
        end
    end
    else begin
        for (int ptr_buffer = 0 ; ptr_buffer < NP ; ptr_buffer = ptr_buffer + 1) begin
            r_fft_input_buffer[ptr_buffer] = r_fft_input_buffer[ptr_buffer];
            // r_fft_input_buffer_re[ptr_buffer] = r_fft_input_buffer_re[ptr_buffer];
            // r_fft_input_buffer_im[ptr_buffer] = r_fft_input_buffer_im[ptr_buffer];
        end
    end
end
/// 
...
// assign r_fft_data_in_next    = (r_fft_data_in_ld) ? {r_fft_input_buffer_im[r_index_reg],r_fft_input_buffer_re[r_index_reg]} : r_fft_data_in_reg;
assign r_fft_data_in_next    = (r_fft_data_in_ld) ? r_fft_input_buffer[r_index_reg] : r_fft_data_in_reg;
// fft runtime config
//assign NFFT                  = $clog2(NP);                         // select nfft size
...
assign SCALE_SCH             = 6'b010111;        // select appropiate scaling factor for N = 64 
//assign w_s_axis_config_tdata = {SCALE_SCH,FWD_INV,{3{1'b0}},NFFT}; // load config
...
always_comb begin : fsmd_next_state_output_logic
    ...
    case (state_reg)
        ...
        S_STR_RAM : begin
            // ADDED SIGNAL TO STOP INPUT WINDOW FROM WORKING AL PEDO
            r_input_win_enable = '1;
            ...
            if (i_window_prod_valid_re && i_window_prod_valid_im) begin // valid output from input_window
                ...
                r_ram_write_addr_inc = '0;
                // added porq si no estan los valid no sabe a donde ir
                state_next = S_STR_RAM;
            end   
        end
        S_LD_BUFF : begin 
            ...
            // load ram data into fft input buffer
            r_fft_input_buffer_ld = '1;
            //for (int ptr_buffer = 0 ; ptr_buffer < NP ; ptr_buffer = ptr_buffer + 1) begin
            //    r_fft_input_buffer_re[ptr_buffer] = i_ram_data_out_re[ptr_buffer];
            //    r_fft_input_buffer_im[ptr_buffer] = i_ram_data_out_im[ptr_buffer];
            //end
        ...
        S_WAIT_NXT_BLOCK : begin
            w_ram_portb_enable     = '1; // !!!!!!!!!!!
            // test for better index management
            // r_index_inc = '0;
            // como esta el clr con prioridad, da igual, para evitar switch lo dejo
            // --------------------------------
            r_ram_read_addr_clr = '0; // !!!!!!!!
...
// set all outputs as wires (after reg)
